...
class abonne{
	...
	static function ajouterAbonneAdmin($bdd, $cni, $nom, $prenom, $addr, $tel, $date,$frais){
		$timestamp=strtotime($date);
		$requete = " INSERT INTO abonne (prenom,nom,cni,telephone,adresse,date_abonnement,timestamp,frais_abonnement) VALUES('$prenom','$nom','$cni','$tel','$addr','$date','$timestamp','$frais')";
	...
	static function ajouterAbonneGerant($bdd, $cni, $nom, $prenom, $adresse, $tel, $date,$ancien_index,$frais){
		$timestamp=strtotime($date);
		$req=$bdd->query("INSERT INTO attente (prenom,nom,cni,telephone,adresse,date_abonnement,timestamp,frais_abonnement,ancien_index) VALUES('$prenom','$nom','$cni','$tel','$adresse','$date','$timestamp','$frais','$ancien_index')");
	}
	
	static function validerAbonne($bdd, $cni,$choix){
		if($choix=="Oui"){
			...
			//initialisation du compteur de l'abonne
			$requete2 = "SELECT id_abonne,date_abonnement,adresse FROM abonne WHERE cni='$cni' AND prenom='$prenom' AND nom='$nom'";
			$resultat2= $bdd->query($requete2);
			while($donnes=$resultat2->fetch()){
				$id_abonne=$donnes['id_abonne']; // clé secondaire : permet de lier un abonné à un compteur
				//$ancien_index=0;
	...
	function supprimerAbonne($bdd){
		...
		$req1=$bdd->prepare("DELETE from compteur WHERE id=:n2");
		$req1->execute(array(
			':n2'=>$this->id,
			));
	}
...
class Compteur{
	...
	function nouveauCompteur($ancien_index,$nouvel_index,$date_index,$special){
		$etat=0;
	...
	function facture($id_index){
		...
		if ($donnees = $reponse->fetch())
		{
			...
			if($donnees['adresse']=="village_special")
				$tarifApplique=$tarifSpecial;
			$montant= $diff*$tarifApplique;
			...
			$resultat=array('id_compteur'=>$donnees['id_compteur'],
				'id_abonne'=>$donnees['id_abonne'],
				'prenom_maj'=>$donnees['prenom_maj'],
				'nom_maj'=>$donnees['nom_maj'],
				'adresse'=>$donnees['adresse'],
				'date_index'=$donnees['date'],
				'periode'=>$periode,
				'ancien_index'=>$donnees['ancien_index'],
				'nouvel_index'=>$donnees['nouvel_index'],
				'diff'=>$diff,
				'tarif'
				);
...
class Finance{
	...
	function superviserRecetteTotal($time1,$time2){
		if($time1<=$time2){
			...
			$facture_total=($volum2*$tarifSpecial)+($volum*$tarif);
				//*********
...
function consommationPeriode($timestamp1,$timestamp2){
			//"SELECT SUM(nouvel_index-ancien_index) AS total_index FROM compteur WHERE timestamp >='$timestamp1' AND timestamp <='$timestamp2' "
			$requet = "SELECT nouvel_index,ancien_index FROM compteur WHERE special=0 AND timestamp>='$timestamp1' AND timestamp<='$timestamp2' ";
			$resulta= $this->bdd->query($requet);
			$facture_totalNormal=0;
			$volum=0;
			while($don=$resulta->fetch())
			...
			$requet2 = "SELECT nouvel_index,ancien_index FROM compteur WHERE special=1 AND timestamp>='$timestamp1' AND timestamp<='$timestamp2'";
			$resulta2= $this->bdd->query($requet2);
			$facture_totalSpecial=0;
			$volum2=0;
			while($don2=$resulta2->fetch())
